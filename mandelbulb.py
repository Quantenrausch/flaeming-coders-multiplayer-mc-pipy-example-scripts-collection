...
# setting this to 0 makes for more isolated blocks because the connecting
# filigree was
...
#
# we could of course just do for x in range(0,size): for y in range(0,size):
# yield(x,y)
...
if __name__ == '__main__':
	...
	while True:
		...
		while not pollZoom():
			time.sleep(0.25)
		if (lastHitEvent.pos.x < cornerMC.x or lastHitEvent.pos.x >= cornerMC.x + size or lastHitEvent.pos.y < cornerMC.y or lastHitEvent.pos.y >= cornerMC.y + size or lastHitEvent.pos.z < cornerMC.z or lastHitEvent.pos.z >= cornerMC.z + size):
